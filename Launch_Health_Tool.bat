...
:: Not admin - relaunch with UAC elevation request
...
:run
set BAT_DIR=%~dp0
powershell.exe -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -Command "& ([ScriptBlock]::Create((Get-Content -Raw -LiteralPath '%~f0')))"
exit /b
#>
...
.NOTES    HOW TO RUN (replaces the old .vbs launcher):
             Right-click this file → "Run with PowerShell"
             -or-
             powershell.exe -ExecutionPolicy Bypass -File ".\Simple_System_Health_Tool.ps1"

             To make double-click work without right-clicking, run once in an
             admin PowerShell:
